...
export function MobileWorkOrders() {
  ...
  const fetchData = async () => {
    try {
      const [workOrdersResponse, vehiclesResponse] = await Promise.all([
        apiRequest('/api/maintenance/work-orders'),
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await apiRequest('/api/maintenance/work-orders', {
        method: 'POST',
      ...
      if (response && (response as Response).ok) {
        ...
        const errorData = response ? await (response as Response).text() : 'No response';
        console.error('Failed to submit work order:', status, errorData);
      }
    } catch (error) {
